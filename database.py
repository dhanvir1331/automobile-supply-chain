...
def create_order(db: Session, customer_order: str):
    db_order = Order(
        customer_order=customer_order, status="Pending", processed_by="Customer"
    )
    db.add(db_order)
    db.commit()
    db.refresh(db_order)
    ...
    for material in materials_needed:
        try:
            raw_material = (
                ...
                .filter(RawMaterial.material_name == material)
                .one()
            )
...
def check_inventory_levels(threshold=100):
    db = SessionLocal()
    low_materials = (
        db.query(RawMaterial).filter(RawMaterial.quantity_available < threshold).all()
    )
    db.close()
    ...
    if low_materials:
        for material in low_materials:
            print(
                f"Warning: {material.material_name} is low (Available: {material.quantity_available})"
            )
    else:
