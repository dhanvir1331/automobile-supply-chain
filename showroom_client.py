        ...
        try:
            ...
            if message:
                ...
                if status == "Distributed":
                    db = SessionLocal()
                    db_order = update_order_status(db, int(order_id), "In Showroom", "Showroom")
...
def process_order():
    if order_queue:
        ...
        client_socket.send(f"{processed_order.id}:{processed_order.customer_order}:Completed".encode('utf-8'))
        update_order_status(SessionLocal(), processed_order.id, "Completed", "Showroom")
        update_order_listbox()
    if not order_queue:
...
client_socket.send('showroom'.encode('utf-8'))

root = tk.Tk()
root.title("Showroom Client")
