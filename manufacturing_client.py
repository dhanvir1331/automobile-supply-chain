            ...
            if message:
                ...
                if status == "Ordered":
                    ...
                    db_order = update_order_status(
                        db, int(order_id), "In Manufacturing", "Manufacturing"
                    )
...
def process_order():
    if order_queue:
        ...
        if check_raw_materials(processed_order.customer_order):
            ...
            client_socket.send(
                f"{processed_order.id}:{processed_order.customer_order}:Manufactured".encode(
                    "utf-8"
                )
            )
            update_order_status(
                SessionLocal(), processed_order.id, "Processed", "Manufacturing"
            )
            update_order_listbox()
            intermediary_listbox.insert(
                tk.END, f"Order ID: {processed_order.id} Manufactured."
            )
        else:
