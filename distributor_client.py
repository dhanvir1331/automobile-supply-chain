            ...
            if message:
                ...
                if status == "Manufactured":
                    ...
                    db_order = update_order_status(
                        db, int(order_id), "In Distribution", "Distributor"
                    )
...
def process_order():
    if order_queue:
        ...
        client_socket.send(
            f"{processed_order.id}:{processed_order.customer_order}:Distributed".encode(
                "utf-8"
            )
        )
        update_order_status(
            SessionLocal(), processed_order.id, "Processed", "Distributor"
        )
        update_order_listbox()
    if not order_queue:
